...
export default class Display extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            intCP: -1,
            //总共有几页
            pageNum: this.props.movieList.length,
            //记录当前是否为手指拖拽
            ifTouch: false,
        ...
        return (
            <View>
                ...
                <View style={styles.movieInfo}>
                    <Text style={styles.movieName}>{this.props.movieList[this.state.currentPage].movieName}</Text>
                    <Text style={styles.movieActor}>{this.props.movieList[this.state.currentPage].movieActor}</Text>
                    <Text style={styles.movieScore}>评分:{this.props.movieList[this.state.currentPage].movieScore}</Text>
                </View>
                <SessionList current={this.props.movieList[this.state.currentPage].movieId}/>
            </View>
        );
    }
            ...
            if (i === currentPage) {
                arr.push(
                    ...
                    <TouchableOpacity key={i} style={styles.img_center} onLayout={this.imageLayout}>
                        <Image source={{ uri: movieList[i].movieImg }}
